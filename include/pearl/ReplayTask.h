    ...
    : public Task
{
    ...
    protected:
        /// @todo[C++11] Replace by alias template
        template< class derivedT >
        struct callback
        {
            typedef void (derivedT::* type)(const CallbackManager&,
                                            int,
                                            const Event&,
                                            CallbackData*);
        };
        ...
        template< class derivedT >
        void
        register_callback(event_t                             event,
                          typename callback< derivedT >::type fptr);

        template< class derivedT >
        void
        register_callback(int                                 user_event,
                          typename callback< derivedT >::type fptr);
 ...
 *  {
 *      public:
 *          MyReplay(const GlobalDefs& defs,
 *                   LocalTrace&       trace)
 ...
 *      protected:
 *          void
 *          cb_send(const CallbackManager& cbmanager,
 *                  int                    user_event,
 *                  const Event&           event,
 *                  CallbackData*          cdata)
 *          {
...
template< class derivedT >
inline void
ReplayTask::register_callback(event_t                             event,
                              typename callback< derivedT >::type fptr)
{
 ...
 *      protected:
 *          void
 *          cb_dowork(const CallbackManager& cbmanager,
 *                    int                    user_event,
 *                    const Event&           event,
 *                    CallbackData*          cdata)
 *          {
...
template< class derivedT >
inline void
ReplayTask::register_callback(int                                 user_event,
                              typename callback< derivedT >::type fptr)
{
    if (derivedT* d = dynamic_cast< derivedT* const >(this))
    {
