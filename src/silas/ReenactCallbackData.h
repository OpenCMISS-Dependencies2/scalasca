...
#ifndef SILAS_REENACTCALLBACKDATA_H
#define SILAS_REENACTCALLBACKDATA_H
    ...
    : public pearl::CallbackData
{
    ...
    public:
        ...
        typedef std::map< uint64_t, RequestInfo > requestmap_t;
        typedef std::set< uint64_t >              requestset_t;
        ...
        virtual const pearl::GlobalDefs&
        get_global_defs() const;
        ...
        void
        set_delta_t(pearl::timestamp_t timespan);

        void
        mod_delta_t(pearl::timestamp_t timespan);
        ...
        void*
        get_send_buffer(std::size_t capacity);

        void*
        get_p2p_send_buffer(std::size_t capacity);

        void*
        get_recv_buffer(std::size_t capacity);

        silas::Buffer*
        pop_nb_recv_buffer();

        void
        push_nb_recv_buffer(silas::Buffer* buf);

        /// @}
        /// @name Tracking of send/recv information
        /// @{

        /// @todo called with uint64_t argument
        void
        track_max_count(int count);

        int
        get_max_count();

        void
        set_max_count(int count);

        /// @todo called with uint64_t argument
        void
        track_max_pt2pt_count(int count);

        int
        get_max_pt2pt_count();

        void
        set_max_pt2pt_count(int count);

        /// @todo called with uint64_t argument
        void
        track_max_buffered_count(int count);

        int
        get_max_buffered_count();

        void
        track_max_mult_comp_count(int count);

        int
        get_max_mult_comp_count();
        ...
        requestmap_t::iterator*
        get_requestmap_iter_array();
        ...
        pearl::timestamp_t
        get_original_timestamp() const
        {
            return m_original_timestamp;
        }

        void
        set_original_timestamp(pearl::timestamp_t timestamp)
        {
            m_original_timestamp = timestamp;
        }

        pearl::timestamp_t
        get_preceeding_timestamp() const
        {
            return m_preceeding_timestamp;
        }

        void
        set_preceeding_timestamp(pearl::timestamp_t timestamp)
        {
        ...
        void
        activate_action_list();

        void
        deactivate_action_list();

        void
        init_action_list();

        void
        del_action(const pearl::Event& event)
        {
            m_action_list.erase(m_action_list.begin() + event.get_id());
        }

        void
        set_action(const int           action,
        ...
        int
        get_action(const pearl::Event& event) const
        {
            return m_action_list[event.get_id()];
        }

        pearl::Event
        get_prev_event(const pearl::Event& event);

        unsigned int
        get_aggregation_count(const pearl::Event& event) const;
        ...
        void
        track_idle_time(const pearl::Event& event);

        void
        aggregate_idle_time(const pearl::Event& event);

        bool
        is_aggregatable(const pearl::Event& event) const;
        ...
        void
        calculate_timespans(const pearl::Event& event,
                            pearl::timestamp_t  start,
                            pearl::timestamp_t  end);
...
#endif    // !SILAS_REENACTCALLBACKDATA_H
