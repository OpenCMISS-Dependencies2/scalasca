...
class Pattern
{
    public:
        ...
        void
        set_classname(const std::string& name)
        {
            m_class = name;
        }

        void
        set_docname(const std::string& name)
        {
            m_docname = name;
        }

        void
        set_parent(const std::string& id)
        {
            m_parent = id;
        }

        void
        set_info(const std::string& info)
        {
            m_info = info;
        }

        void
        set_descr(const std::string& descr)
        {
            m_descr = descr;
        }

        void
        set_diagnosis(const std::string& diag)
        {
            m_diag = diag;
        }
        ...
        void
        set_condition(const std::string& cond)
        {
            m_cond = cond;
        }

        void
        set_init(const std::string& code)
        {
            m_init = code;
        }

        void
        set_staticinit(const std::string& code)
        {
            m_staticinit = code;
        }

        void
        set_cleanup(const std::string& code)
        {
            m_cleanup = code;
        }

        void
        set_data(const std::string& code)
        {
            m_data = code;
        }
        ...
        skip_impl() const
        {
            return (m_hidden && m_callbacks.empty());
        }

        /* Adding callbacks */
        bool
        add_callback(const std::string& cbgroup,
        ...
        /* Writing output */
        void
        write_impl(std::FILE* fp,
                   int&       indent) const;

        void
        write_html(std::FILE* fp,
                   bool       isFirst);
    ...
    private:
        ...
        std::string m_mode;       // data storage mode (exclusive/inclusive)
        ...
        std::string m_staticinit; // code used for initialization of static data
        ...
        // callbackgroups |-> events
        std::map< std::string, std::vector< std::string > > m_callbackgroups;
