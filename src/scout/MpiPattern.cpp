 ...
 **/
bool
MpiPattern::skip_cnode(const pearl::Callpath& cnode) const
{
    return !is_mpi_api(cnode.getRegion());
}
 ...
 **/
void
MpiPattern::exchange_severities(const pearl::GlobalDefs&   defs,
                                const rem_sev_container_t& remote_sev)
{
    #pragma omp master
    {
        ...
        {
            lcounts[defs.getLocation(lit->first).getRank()]++;
            ++lit;
        }
        ...
        while (lit != remote_sev.end())
        {
            ...
            msg->isend(defs.getLocation(lit->first).getRank(), 42);
