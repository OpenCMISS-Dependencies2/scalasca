...
void
EventSet::add_event(const Event& event,
                    uint32_t     key)
{
...
Event
EventSet::get_event(uint32_t key) const
{
...
void
EventSet::send(const MpiComm& comm,
               int            dest,
               int            tag) const
{
...
MpiMessage*
EventSet::isend(const MpiComm& comm,
                int            dest,
                int            tag) const
{
...
void
EventSet::bcast(const MpiComm& comm,
                int            root) const
{
    MpiMessage message(comm);
...
RemoteEvent
EventSet::allreduce_min(const GlobalDefs& defs,
...
RemoteEvent
EventSet::allreduce_max(const GlobalDefs& defs,
                        const MpiComm&    comm) const
{
    struct
    {
...
void
EventSet::pack(MpiMessage& message) const
{
